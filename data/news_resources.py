...
def abort_if_news_not_found(news_id):
    ...
    if not news:
        flask_restful.abort(404, message=f"News {news_id} not found")
...
class NewsListResource(Resource):
    ...
    def post(self):
        ...
        session = db_session.create_session()
        news = News(title=args["title"], content=args["content"], user_id=args["user_id"], is_private=args["is_private"], is_published=args["is_published"])
        session.add(news)
        session.commit()
